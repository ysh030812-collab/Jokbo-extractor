...
# ── 2022 소화기: 첫머리 문제 두 개에 번호가 빠져 있고 본문은 3번부터 ─────
d = docx.Document()
A = d.add_paragraph
A("2022학년도 2학기 소화기계 시험 문제 복원-인하대학교 의학과 36기")
A("주의사항")
A("*문제는 번호, 마침표, 스페이스바 하고 써주세요")
A("*문제와 문제 사이에 엔터키 한번 쳐주세요.")
A("________________________________________________________________________")
# 여기 둘은 번호가 없다 (원본 그대로)
p1 = A("(김채균) 해당 약물은 somatostatin과 유사한 작용을 보이는 약물로 variceal 의 pressure 를 낮춘다. 무엇인가?")
p1._p.append(parse_xml(NUMPR))
p2 = A("당뇨를 치료 중인 50세 여자 환자가 우상복부 동통 및 발열을 주소로 내원하였다. 진단은 무엇인가?")
p2._p.append(parse_xml(NUMPR))
A("3. (이돈행)췌장두부암의 조직학적 진단에 가장 유용한 검사법은 무엇인가?")
A("5. (윤용한) 식도와 연결하는 부위로 빈도가 높은 것부터 3개 쓰시오.")
# 43번 — 선지가 "1. 2. 3. 4. 5." 라서 4번 문제로 오인되기 쉬운 자리
A("43. 간의 혈관종에 대한 설명으로 옳지 않은 것은?")
for t in ["1. 전암성 병변이다.", "2. 남성이 여성보다 호발한다.", "3. 3cm 이하면 추적관찰 필요없다.",
          "4. rupture가 잘 일어나므로 수술을 반드시 해야한다.", "5. 건강검진 중 우연히 발견하는 경우가 많다."]:
    A(t)
d.save(OUT / "real2022sohwa.docx")


# ── 2023 소화기: 뒤늦게 끼워 넣어 번호가 크게 되돌아간 문제 ──────────────
d = docx.Document()
A = d.add_paragraph
A("2023 소화기 문제복원")
A("1. 0세 여자가 식후 소화불량과 속쓰림 증상이 있어 병원에 왔다. 알맞은 약은?")
choices2 = lambda *xs: [A(t)._p.append(parse_xml(NUMPR)) for t in xs]
choices2("Lansoprazole", "Sucralfate", "Metronidazole")
A("111. 담낭질환의 담낭절제술의 적응증은?(모두 고르시오)")
choices2("다발성 담석", "무증상 담석", "기종성 담낭염")
# 111 뒤에 16번이 끼어 있다 — 번호가 크게 되돌아간다
A("16. [진영주] 간농양에 대한 설명 중 맞는 것은?")
for t in ["1) 가장 흔한 원인균은 staphylococcus aureus이다.",
          "2) K. neumoniae 에 의한 간 농양은 추가 검사가 불필요하다.",
          "3) 세균성 간 농양의 치료는 항생제 하나로 충분하다.",
          "4) 가장 흔한 증상은 설사이다.",
          "5) 흔한 증상은 발열 및 복통이다."]:
    A(t)
A("117. 43세 남자가 목 이물감, 기침, 역류 증세가 1년 넘게 있었다. 옳은 것은?")
choices2("가", "나", "다")
d.save(OUT / "real2023sohwa.docx")


# ── 번호를 묶어 낸 문제 ────────────────────────────────────────────
# "43-45. 알코올성 간질환의 …", "37,38. 위에서 …" 같은 머리는 구분점이 번호
# 바로 뒤에 없어서 세 파서가 모두 놓쳤다. 그 문제가 통째로 사라지고 앞 문제
# 블록에 딸려 들어갔다.
d = docx.Document()
for t in ["36. 다음 중 옳은 것은?", "1) 가", "2) 나",
          "37,38. 위에서 점막하 종양이 보인다. 진단은?", "1) 가", "2) 나",
          "39. 그 다음 문제는?", "1) 가", "2) 나",
          "43-45. 알코올성 간질환의 3가지 질환 스펙트럼을 쓰고 각각의 특징을 간략히 기술하시오.",
          "46. 마지막 문제는?", "1) 가", "2) 나"]:
    d.add_paragraph(t)
d.save(OUT / "range.docx")
