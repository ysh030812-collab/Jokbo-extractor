...
/* 구분점 뒤에 바로 숫자가 오면 문제 번호가 아니라 소수다.
   "38.5도의 발열이 나타나므로…" 가 38번 문제로 잡혀 13~37번이 통째로 날아갔다.
   ".다음" 처럼 공백 없이 이어지는 진짜 문제 머리는 그대로 인정한다. */
const NUMBERED = /^\s*(\d{1,3})\s*번?\s*[.)．](?!\d)\s*(.*)$/;
  ...
  for (const p of paras) {
    ...
    for (const ln of lines) {
      ...
      if (m) {
        ...
        if (isNew(n, m[2])) { startQ(n, m[2]); continue; }
        if (cur) cur._body.push([ln, isList]);
        continue;
      }
      const dm = NUMBERED_DEC.exec(ln);
      if (dm && +dm[1] === maxNum + 1) { startQ(+dm[1], dm[2]); continue; }
      const lm = LOOSE_NUM.exec(ln);
      if (lm && +lm[1] === maxNum + 1 && !COUNTER.test(lm[2])) { startQ(+lm[1], lm[2]); continue; }
      if (cur) {
        /* "126." 처럼 번호만 찍고 지문은 다음 줄에 쓴 족보가 있다. 지문 자리가
           비어 있으면 뒤이어 오는 첫 줄을 지문으로 올린다. 목록 서식이면 선지일
           수 있으니 물음표로 끝날 때만 올린다. */
        if (!cur.stem && !cur._body.length && (!isList || /[?？]\s*$/.test(ln))) {
          cur.stem = ln; continue;
        }
        cur._body.push([ln, isList]);
      } else pre.push([ln, isList]);
    }
    if (imgs.length) (cur ? cur.images : preImgs).push(...imgs);
  }
  /* 주의사항 구분선(______) 뒤에 번호 없이 적혀 있고 본문은 3번부터 시작하는
     족보가 있다. 구분선 뒤 줄 수가 빠진 번호 수와 정확히 맞을 때만 1번부터
     차례로 매긴다 — 어긋나면 손대지 않는다. */
  const firstNum = qs.size ? Math.min(...qs.keys()) : 0;
  if (firstNum >= 2 && pre.length) {
    const at = pre.findIndex(([t]) => /^[_\-—─=＿]{8,}$/.test(t.replace(/\s/g, "")));
    const rest = at >= 0 ? pre.slice(at + 1) : [];
    if (rest.length === firstNum - 1) {
      rest.forEach(([t], i) => qs.set(i + 1, { num: i + 1, stem: t, _body: [], images: [] }));
      if (preImgs.length) qs.get(1).images.push(...preImgs);
    }
  }
...
pages 는 근거가 된 **강의안 쪽 번호**입니다. 숫자만 씁니다 — "8", "8-9", "8, 12".
슬라이드 제목이나 소제목을 적지 마세요 ("Congenital Anomaly" 처럼 쓰면 안 됩니다).
쪽 번호를 알 수 없으면 빈 칸("")으로 두세요.
